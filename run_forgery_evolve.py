...
def wevade(args, decoder, device, preprocess, meter, logger):
    ...
    wm_sub_path = os.path.join(wm_imgs_path, "set_A")
    wm_load_path = [os.path.join(wm_sub_path, file_name) for file_name in os.listdir(wm_sub_path)]
    wm_sub_path = os.path.join(wm_imgs_path, "set_B")
    wm_load_path += [os.path.join(wm_sub_path, file_name) for file_name in os.listdir(wm_sub_path)]
    wm_sub_path = os.path.join(wm_imgs_path, "set_C")
    wm_load_path += [os.path.join(wm_sub_path, file_name) for file_name in os.listdir(wm_sub_path)]
    images = [Image.open(image_path).convert("RGB") for image_path in wm_load_path]
    ...
    for img_id, x in enumerate(images):
        ...
        for i in range(iters):
            ...
            #     break
            criterion = (bit_acc_target <= epsilon) if 'w0' in args.wi else (bit_acc_target >= 1 - epsilon)
            if criterion:
...
def main(args):
    ...
    for i, subdir in enumerate(os.listdir(data_path)):
        ...
        args.wm_imgs_path = os.path.join(data_path, subdir)
