...
def initialize_prompt(tokenizer, token_embedding, args, device, prefix_list=None):
    ...
    if prefix_list is not None:
        prompt_len, prefix_embeds = [], []
        for prefix in prefix_list:
            prefix_ids = tokenizer.encode(prefix)
            prefix_ids = torch.tensor([prefix_ids]).to(device)
            prefix_embed = token_embedding(prefix_ids).detach()
            prefix_embed.requires_grad = False
            prefix_embeds.append(prefix_embed)
            prompt_len.append(args.prompt_len + prefix_embed.shape[1])
    else:
        prefix_embeds = None
    ...
    for i in range(len(prefix_list)):
        ...
        template_text = "{}"
        padded_template_text = template_text.format(" ".join(["<start_of_text>"] * prompt_len[i]))
        dummy_id = tokenizer.encode(padded_template_text)

        # -1 for optimized tokens
        dummy_id = [i if i != 49406 else -1 for i in dummy_id]
        dummy_id = [49406] + dummy_id + [49407]
        dummy_id += [0] * (77 - len(dummy_id))
    ...
    return prompt_embeds, dummy_embeds, dummy_ids, prefix_embeds


def optimize_prompt_loop(model, tokenizer, token_embedding, all_target_features, args, device, prefix=None):
    opt_iters = args.iters
    lr = args.lr
    ...
    # initialize prompt
    # dummy embeddings are used for padding the prompt_embeds
    prompt_embeds, dummy_embeds, dummy_ids, prefix_embeds = initialize_prompt(tokenizer, token_embedding, args, device, prefix)
    ...
    for step in range(opt_iters):
        ...
        with torch.no_grad():
            ...
            for i in range(len(prefix_embeds)):
                concat_embeds = torch.cat([prefix_embeds[i], projected_embeds], dim=1) if prefix_embeds is not None else projected_embeds
                padded_embeds[i][dummy_ids[i] == -1] = concat_embeds.reshape(-1, p_dim).squeeze(0)
        ...
        tmp_embeds = projected_embeds.detach().clone()
        ...
        padded_embeds = dummy_embeds.detach().clone()
        for i in range(len(prefix_embeds)):
            concat_embeds = torch.cat([prefix_embeds[i], tmp_embeds], dim=1) if prefix_embeds is not None else tmp_embeds
            padded_embeds[i][dummy_ids[i] == -1] = concat_embeds.reshape(-1, p_dim).squeeze(0)
        ...
        prompt_embeds.grad, = (torch.mean(torch.autograd.grad(loss, [tmp_embeds])[0], dim=0).unsqueeze(0), )
...
def optimize_prompt(model, preprocess, args, device, target_images=None, target_prompts=None, prefix=None):
    ...
    # optimize prompt
    learned_prompt = optimize_prompt_loop(model, tokenizer, token_embedding, all_target_features, args, device, prefix)
