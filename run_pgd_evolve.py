...
def pgd(x, x_trans, model, vae, alpha, eps, iters=100):
    ...
    for i in pbar:
        ...
        sim_loss = criterion(x_emb, x_trans_emb)
        rec_loss = criterion(vae(x_adv).sample, x_adv)
        loss = sim_loss + 1e2 * rec_loss
        ...
        # scheduler.step()
        pbar.set_description(f"[Running pgd]: Sim Loss {sim_loss.item():.5f} | Rec Loss {rec_loss.item():.5f} | noise norm {torch.norm(eta, p=2):.5f}")
