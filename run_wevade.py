    ...
    for img_id, x in enumerate(images):
        ...
        with torch.no_grad():
            ...
            if 'w0' in args.wi or 'w1' in args.wi:
                bit_acc = 0
                for _ in range(10):
                    noise = torch.randn(x.size()).to(device) * 0.1
        ...
        if ('w0' in args.wi and bit_acc_orig < args.tau) or ('w1' in args.wi and bit_acc_orig > args.tau):
            evasion = (bit_acc >= args.tau) if 'w0' in args.wi else (bit_acc <= args.tau)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_name', type=str, default='VGGFace2_wevade_e0')
    ...
    # watermark params
    parser.add_argument("--num_bits", type=int, default=32, help="Number of bits of the watermark (Default: 32)")
